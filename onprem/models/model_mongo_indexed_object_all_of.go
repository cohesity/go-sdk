/*
 * Cohesity REST API
 *
 * Cohesity API provides a RESTful interface to access the various data management operations on Cohesity cluster and Helios.
 *
 * API version: 2.0
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

// package onprem
package models

import (
	"encoding/json"
	. "github.com/cohesity/go-sdk/onprem/utils"
	"fmt"
)

var _ = NullableBool{}

// MongoIndexedObjectAllOf struct for MongoIndexedObjectAllOf
type MongoIndexedObjectAllOf struct {
	// Specifies the Mongo Object Type.
	Type NullableString `json:"type,omitempty"`
	// Specifies the id of the indexed object.
	Id NullableString `json:"id,omitempty"`
	CdpInfo *CdpObjectInfo `json:"cdpInfo,omitempty"`
}

// NewMongoIndexedObjectAllOf instantiates a new MongoIndexedObjectAllOf object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMongoIndexedObjectAllOf() *MongoIndexedObjectAllOf {
	this := MongoIndexedObjectAllOf{}
	return &this
}

// NewMongoIndexedObjectAllOfWithDefaults instantiates a new MongoIndexedObjectAllOf object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMongoIndexedObjectAllOfWithDefaults() *MongoIndexedObjectAllOf {
	this := MongoIndexedObjectAllOf{}
	return &this
}

// GetType returns the Type field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MongoIndexedObjectAllOf) GetType() string {
	if o == nil || o.Type.Get() == nil {
		var ret string
		return ret
	}
	return *o.Type.Get()
}

// GetTypeOk returns a tuple with the Type field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MongoIndexedObjectAllOf) GetTypeOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.Type.Get(), o.Type.IsSet()
}

// HasType returns a boolean if a field has been set.
func (o *MongoIndexedObjectAllOf) HasType() bool {
	if o != nil && o.Type.IsSet() {
		return true
	}

	return false
}

// SetType gets a reference to the given NullableString and assigns it to the Type field.
func (o *MongoIndexedObjectAllOf) SetType(v string) {
	o.Type.Set(&v)
}
// SetTypeNil sets the value for Type to be an explicit nil
func (o *MongoIndexedObjectAllOf) SetTypeNil() {
	o.Type.Set(nil)
}

// UnsetType ensures that no value is present for Type, not even an explicit nil
func (o *MongoIndexedObjectAllOf) UnsetType() {
	o.Type.Unset()
}

// GetId returns the Id field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MongoIndexedObjectAllOf) GetId() string {
	if o == nil || o.Id.Get() == nil {
		var ret string
		return ret
	}
	return *o.Id.Get()
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MongoIndexedObjectAllOf) GetIdOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.Id.Get(), o.Id.IsSet()
}

// HasId returns a boolean if a field has been set.
func (o *MongoIndexedObjectAllOf) HasId() bool {
	if o != nil && o.Id.IsSet() {
		return true
	}

	return false
}

// SetId gets a reference to the given NullableString and assigns it to the Id field.
func (o *MongoIndexedObjectAllOf) SetId(v string) {
	o.Id.Set(&v)
}
// SetIdNil sets the value for Id to be an explicit nil
func (o *MongoIndexedObjectAllOf) SetIdNil() {
	o.Id.Set(nil)
}

// UnsetId ensures that no value is present for Id, not even an explicit nil
func (o *MongoIndexedObjectAllOf) UnsetId() {
	o.Id.Unset()
}

// GetCdpInfo returns the CdpInfo field value if set, zero value otherwise.
func (o *MongoIndexedObjectAllOf) GetCdpInfo() CdpObjectInfo {
	if o == nil || o.CdpInfo == nil {
		var ret CdpObjectInfo
		return ret
	}
	return *o.CdpInfo
}

// GetCdpInfoOk returns a tuple with the CdpInfo field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MongoIndexedObjectAllOf) GetCdpInfoOk() (*CdpObjectInfo, bool) {
	if o == nil || o.CdpInfo == nil {
		return nil, false
	}
	return o.CdpInfo, true
}

// HasCdpInfo returns a boolean if a field has been set.
func (o *MongoIndexedObjectAllOf) HasCdpInfo() bool {
	if o != nil && o.CdpInfo != nil {
		return true
	}

	return false
}

// SetCdpInfo gets a reference to the given CdpObjectInfo and assigns it to the CdpInfo field.
func (o *MongoIndexedObjectAllOf) SetCdpInfo(v CdpObjectInfo) {
	o.CdpInfo = &v
}

func (o MongoIndexedObjectAllOf) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Type.IsSet() {
		toSerialize["type"] = o.Type.Get()
	}
	if o.Id.IsSet() {
		toSerialize["id"] = o.Id.Get()
	}
	if o.CdpInfo != nil {
		toSerialize["cdpInfo"] = o.CdpInfo
	}
	return json.Marshal(toSerialize)
}

type NullableMongoIndexedObjectAllOf struct {
	value *MongoIndexedObjectAllOf
	isSet bool
}

func (v NullableMongoIndexedObjectAllOf) Get() *MongoIndexedObjectAllOf {
	return v.value
}

func (v *NullableMongoIndexedObjectAllOf) Set(val *MongoIndexedObjectAllOf) {
	v.value = val
	v.isSet = true
}

func (v NullableMongoIndexedObjectAllOf) IsSet() bool {
	return v.isSet
}

func (v *NullableMongoIndexedObjectAllOf) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMongoIndexedObjectAllOf(val *MongoIndexedObjectAllOf) *NullableMongoIndexedObjectAllOf {
	return &NullableMongoIndexedObjectAllOf{value: val, isSet: true}
}

func (v NullableMongoIndexedObjectAllOf) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMongoIndexedObjectAllOf) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}




func (o MongoIndexedObjectAllOf) Print() {
		if byteArray, err := o.MarshalJSON(); err != nil {
				fmt.Println(err)
		} else {
				fmt.Println(string(byteArray))
		}
}