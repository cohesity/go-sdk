/*
Cohesity REST API

Cohesity API provides a RESTful interface to access the various data management operations on Cohesity cluster and Helios.

API version: 2.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package v2

import (
	"encoding/json"
)

// checks if the VmwareThrottlingParams type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &VmwareThrottlingParams{}

// VmwareThrottlingParams Specifies throttling params.
type VmwareThrottlingParams struct {
	// If the latency of a datastore is above this value, then an existing backup task that uses the datastore will start getting throttled.
	ActiveTaskLatencyThresholdMsecs NullableInt64 `json:"activeTaskLatencyThresholdMsecs,omitempty"`
	// Specifies the number of VMs of a vCenter that can be backed up concurrently.
	MaxConcurrentBackups NullableInt32 `json:"maxConcurrentBackups,omitempty"`
	// If this value is > 0 and the number of streams concurrently active on a datastore is equal to it, then any further requests to access the datastore would be denied until the number of active streams reduces. This applies for all the datastores in the specified host.
	MaxConcurrentStreams NullableInt32 `json:"maxConcurrentStreams,omitempty"`
	// If the latency of a datastore is above this value, then a new backup task that uses the datastore won't be started.
	NewTaskLatencyThresholdMsecs NullableInt64 `json:"newTaskLatencyThresholdMsecs,omitempty"`
}

// NewVmwareThrottlingParams instantiates a new VmwareThrottlingParams object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewVmwareThrottlingParams() *VmwareThrottlingParams {
	this := VmwareThrottlingParams{}
	return &this
}

// NewVmwareThrottlingParamsWithDefaults instantiates a new VmwareThrottlingParams object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewVmwareThrottlingParamsWithDefaults() *VmwareThrottlingParams {
	this := VmwareThrottlingParams{}
	return &this
}

// GetActiveTaskLatencyThresholdMsecs returns the ActiveTaskLatencyThresholdMsecs field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *VmwareThrottlingParams) GetActiveTaskLatencyThresholdMsecs() int64 {
	if o == nil || IsNil(o.ActiveTaskLatencyThresholdMsecs.Get()) {
		var ret int64
		return ret
	}
	return *o.ActiveTaskLatencyThresholdMsecs.Get()
}

// GetActiveTaskLatencyThresholdMsecsOk returns a tuple with the ActiveTaskLatencyThresholdMsecs field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *VmwareThrottlingParams) GetActiveTaskLatencyThresholdMsecsOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return o.ActiveTaskLatencyThresholdMsecs.Get(), o.ActiveTaskLatencyThresholdMsecs.IsSet()
}

// HasActiveTaskLatencyThresholdMsecs returns a boolean if a field has been set.
func (o *VmwareThrottlingParams) HasActiveTaskLatencyThresholdMsecs() bool {
	if o != nil && o.ActiveTaskLatencyThresholdMsecs.IsSet() {
		return true
	}

	return false
}

// SetActiveTaskLatencyThresholdMsecs gets a reference to the given NullableInt64 and assigns it to the ActiveTaskLatencyThresholdMsecs field.
func (o *VmwareThrottlingParams) SetActiveTaskLatencyThresholdMsecs(v int64) {
	o.ActiveTaskLatencyThresholdMsecs.Set(&v)
}
// SetActiveTaskLatencyThresholdMsecsNil sets the value for ActiveTaskLatencyThresholdMsecs to be an explicit nil
func (o *VmwareThrottlingParams) SetActiveTaskLatencyThresholdMsecsNil() {
	o.ActiveTaskLatencyThresholdMsecs.Set(nil)
}

// UnsetActiveTaskLatencyThresholdMsecs ensures that no value is present for ActiveTaskLatencyThresholdMsecs, not even an explicit nil
func (o *VmwareThrottlingParams) UnsetActiveTaskLatencyThresholdMsecs() {
	o.ActiveTaskLatencyThresholdMsecs.Unset()
}

// GetMaxConcurrentBackups returns the MaxConcurrentBackups field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *VmwareThrottlingParams) GetMaxConcurrentBackups() int32 {
	if o == nil || IsNil(o.MaxConcurrentBackups.Get()) {
		var ret int32
		return ret
	}
	return *o.MaxConcurrentBackups.Get()
}

// GetMaxConcurrentBackupsOk returns a tuple with the MaxConcurrentBackups field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *VmwareThrottlingParams) GetMaxConcurrentBackupsOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.MaxConcurrentBackups.Get(), o.MaxConcurrentBackups.IsSet()
}

// HasMaxConcurrentBackups returns a boolean if a field has been set.
func (o *VmwareThrottlingParams) HasMaxConcurrentBackups() bool {
	if o != nil && o.MaxConcurrentBackups.IsSet() {
		return true
	}

	return false
}

// SetMaxConcurrentBackups gets a reference to the given NullableInt32 and assigns it to the MaxConcurrentBackups field.
func (o *VmwareThrottlingParams) SetMaxConcurrentBackups(v int32) {
	o.MaxConcurrentBackups.Set(&v)
}
// SetMaxConcurrentBackupsNil sets the value for MaxConcurrentBackups to be an explicit nil
func (o *VmwareThrottlingParams) SetMaxConcurrentBackupsNil() {
	o.MaxConcurrentBackups.Set(nil)
}

// UnsetMaxConcurrentBackups ensures that no value is present for MaxConcurrentBackups, not even an explicit nil
func (o *VmwareThrottlingParams) UnsetMaxConcurrentBackups() {
	o.MaxConcurrentBackups.Unset()
}

// GetMaxConcurrentStreams returns the MaxConcurrentStreams field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *VmwareThrottlingParams) GetMaxConcurrentStreams() int32 {
	if o == nil || IsNil(o.MaxConcurrentStreams.Get()) {
		var ret int32
		return ret
	}
	return *o.MaxConcurrentStreams.Get()
}

// GetMaxConcurrentStreamsOk returns a tuple with the MaxConcurrentStreams field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *VmwareThrottlingParams) GetMaxConcurrentStreamsOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.MaxConcurrentStreams.Get(), o.MaxConcurrentStreams.IsSet()
}

// HasMaxConcurrentStreams returns a boolean if a field has been set.
func (o *VmwareThrottlingParams) HasMaxConcurrentStreams() bool {
	if o != nil && o.MaxConcurrentStreams.IsSet() {
		return true
	}

	return false
}

// SetMaxConcurrentStreams gets a reference to the given NullableInt32 and assigns it to the MaxConcurrentStreams field.
func (o *VmwareThrottlingParams) SetMaxConcurrentStreams(v int32) {
	o.MaxConcurrentStreams.Set(&v)
}
// SetMaxConcurrentStreamsNil sets the value for MaxConcurrentStreams to be an explicit nil
func (o *VmwareThrottlingParams) SetMaxConcurrentStreamsNil() {
	o.MaxConcurrentStreams.Set(nil)
}

// UnsetMaxConcurrentStreams ensures that no value is present for MaxConcurrentStreams, not even an explicit nil
func (o *VmwareThrottlingParams) UnsetMaxConcurrentStreams() {
	o.MaxConcurrentStreams.Unset()
}

// GetNewTaskLatencyThresholdMsecs returns the NewTaskLatencyThresholdMsecs field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *VmwareThrottlingParams) GetNewTaskLatencyThresholdMsecs() int64 {
	if o == nil || IsNil(o.NewTaskLatencyThresholdMsecs.Get()) {
		var ret int64
		return ret
	}
	return *o.NewTaskLatencyThresholdMsecs.Get()
}

// GetNewTaskLatencyThresholdMsecsOk returns a tuple with the NewTaskLatencyThresholdMsecs field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *VmwareThrottlingParams) GetNewTaskLatencyThresholdMsecsOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return o.NewTaskLatencyThresholdMsecs.Get(), o.NewTaskLatencyThresholdMsecs.IsSet()
}

// HasNewTaskLatencyThresholdMsecs returns a boolean if a field has been set.
func (o *VmwareThrottlingParams) HasNewTaskLatencyThresholdMsecs() bool {
	if o != nil && o.NewTaskLatencyThresholdMsecs.IsSet() {
		return true
	}

	return false
}

// SetNewTaskLatencyThresholdMsecs gets a reference to the given NullableInt64 and assigns it to the NewTaskLatencyThresholdMsecs field.
func (o *VmwareThrottlingParams) SetNewTaskLatencyThresholdMsecs(v int64) {
	o.NewTaskLatencyThresholdMsecs.Set(&v)
}
// SetNewTaskLatencyThresholdMsecsNil sets the value for NewTaskLatencyThresholdMsecs to be an explicit nil
func (o *VmwareThrottlingParams) SetNewTaskLatencyThresholdMsecsNil() {
	o.NewTaskLatencyThresholdMsecs.Set(nil)
}

// UnsetNewTaskLatencyThresholdMsecs ensures that no value is present for NewTaskLatencyThresholdMsecs, not even an explicit nil
func (o *VmwareThrottlingParams) UnsetNewTaskLatencyThresholdMsecs() {
	o.NewTaskLatencyThresholdMsecs.Unset()
}

func (o VmwareThrottlingParams) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o VmwareThrottlingParams) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.ActiveTaskLatencyThresholdMsecs.IsSet() {
		toSerialize["activeTaskLatencyThresholdMsecs"] = o.ActiveTaskLatencyThresholdMsecs.Get()
	}
	if o.MaxConcurrentBackups.IsSet() {
		toSerialize["maxConcurrentBackups"] = o.MaxConcurrentBackups.Get()
	}
	if o.MaxConcurrentStreams.IsSet() {
		toSerialize["maxConcurrentStreams"] = o.MaxConcurrentStreams.Get()
	}
	if o.NewTaskLatencyThresholdMsecs.IsSet() {
		toSerialize["newTaskLatencyThresholdMsecs"] = o.NewTaskLatencyThresholdMsecs.Get()
	}
	return toSerialize, nil
}

type NullableVmwareThrottlingParams struct {
	value *VmwareThrottlingParams
	isSet bool
}

func (v NullableVmwareThrottlingParams) Get() *VmwareThrottlingParams {
	return v.value
}

func (v *NullableVmwareThrottlingParams) Set(val *VmwareThrottlingParams) {
	v.value = val
	v.isSet = true
}

func (v NullableVmwareThrottlingParams) IsSet() bool {
	return v.isSet
}

func (v *NullableVmwareThrottlingParams) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableVmwareThrottlingParams(val *VmwareThrottlingParams) *NullableVmwareThrottlingParams {
	return &NullableVmwareThrottlingParams{value: val, isSet: true}
}

func (v NullableVmwareThrottlingParams) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableVmwareThrottlingParams) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


