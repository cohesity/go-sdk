/*
Cohesity REST API

Cohesity API provides a RESTful interface to access the various data management operations on Cohesity cluster and Helios.

API version: 2.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package v2

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the AdSfu30TypeParams type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AdSfu30TypeParams{}

// AdSfu30TypeParams Specifies the properties associated to a Sfu30 type user id mapping.
type AdSfu30TypeParams struct {
	FallbackOption NullableAdLdapProviderTypeParamsFallbackOption `json:"fallbackOption"`
}

type _AdSfu30TypeParams AdSfu30TypeParams

// NewAdSfu30TypeParams instantiates a new AdSfu30TypeParams object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAdSfu30TypeParams(fallbackOption NullableAdLdapProviderTypeParamsFallbackOption) *AdSfu30TypeParams {
	this := AdSfu30TypeParams{}
	this.FallbackOption = fallbackOption
	return &this
}

// NewAdSfu30TypeParamsWithDefaults instantiates a new AdSfu30TypeParams object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAdSfu30TypeParamsWithDefaults() *AdSfu30TypeParams {
	this := AdSfu30TypeParams{}
	return &this
}

// GetFallbackOption returns the FallbackOption field value
// If the value is explicit nil, the zero value for AdLdapProviderTypeParamsFallbackOption will be returned
func (o *AdSfu30TypeParams) GetFallbackOption() AdLdapProviderTypeParamsFallbackOption {
	if o == nil || o.FallbackOption.Get() == nil {
		var ret AdLdapProviderTypeParamsFallbackOption
		return ret
	}

	return *o.FallbackOption.Get()
}

// GetFallbackOptionOk returns a tuple with the FallbackOption field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AdSfu30TypeParams) GetFallbackOptionOk() (*AdLdapProviderTypeParamsFallbackOption, bool) {
	if o == nil {
		return nil, false
	}
	return o.FallbackOption.Get(), o.FallbackOption.IsSet()
}

// SetFallbackOption sets field value
func (o *AdSfu30TypeParams) SetFallbackOption(v AdLdapProviderTypeParamsFallbackOption) {
	o.FallbackOption.Set(&v)
}

func (o AdSfu30TypeParams) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AdSfu30TypeParams) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["fallbackOption"] = o.FallbackOption.Get()
	return toSerialize, nil
}

func (o *AdSfu30TypeParams) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"fallbackOption",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varAdSfu30TypeParams := _AdSfu30TypeParams{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varAdSfu30TypeParams)

	if err != nil {
		return err
	}

	*o = AdSfu30TypeParams(varAdSfu30TypeParams)

	return err
}

type NullableAdSfu30TypeParams struct {
	value *AdSfu30TypeParams
	isSet bool
}

func (v NullableAdSfu30TypeParams) Get() *AdSfu30TypeParams {
	return v.value
}

func (v *NullableAdSfu30TypeParams) Set(val *AdSfu30TypeParams) {
	v.value = val
	v.isSet = true
}

func (v NullableAdSfu30TypeParams) IsSet() bool {
	return v.isSet
}

func (v *NullableAdSfu30TypeParams) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAdSfu30TypeParams(val *AdSfu30TypeParams) *NullableAdSfu30TypeParams {
	return &NullableAdSfu30TypeParams{value: val, isSet: true}
}

func (v NullableAdSfu30TypeParams) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAdSfu30TypeParams) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


