...
type SourceRegistrationPatchRequestParams struct {
	...
	// Required: true
	// Enum: [kVMware kHyperV kAcropolis kKVM kAWS kGCP kAzure kPhysical kPure kIbmFlashSystem kNimble kNetapp kNutanixFS kGenericNas kIsilon kFlashBlade kGPFS kElastifile kO365 kHyperFlex kKubernetes kCassandra kMongoDB kCouchbase kHdfs kHive kHBase kSAPHANA kUDA kSQL kOracle kS3Compatible kSfdc kExperimentalAdapter kMongoDBPhysical kGoogleWorkspace kDB2 kEwsExchange kServiceNow kSalesforce kPostgres kJira]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["kVMware","kHyperV","kAcropolis","kKVM","kAWS","kGCP","kAzure","kPhysical","kPure","kIbmFlashSystem","kNimble","kNetapp","kNutanixFS","kGenericNas","kIsilon","kFlashBlade","kGPFS","kElastifile","kO365","kHyperFlex","kKubernetes","kCassandra","kMongoDB","kCouchbase","kHdfs","kHive","kHBase","kSAPHANA","kUDA","kSQL","kOracle","kS3Compatible","kSfdc","kExperimentalAdapter","kMongoDBPhysical","kGoogleWorkspace","kDB2","kEwsExchange","kServiceNow","kSalesforce","kPostgres","kJira"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
