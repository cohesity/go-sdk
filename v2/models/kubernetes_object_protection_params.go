...
type KubernetesObjectProtectionParams struct {
	...
	// Array of arrays of label IDs that specify labels to exclude. Optionally specify a list of labels to exclude from protecting by listing protection source ids of labels in this two dimensional array. Using this two dimensional array of label IDs, the Cluster generates a list of namespaces to exclude from protecting, which are derived from intersections of the inner arrays and union of the outer array. This is not supported for object protection on helios.
	ExcludeLabelIds [][]int64 `json:"excludeLabelIds"`

	// Specifies the object ids to be excluded for protection. This is not supported for object protection on helios.
	// Unique: true
	...
	// Array of array of label IDs that specify labels to protect. Optionally specify a list of labels to protect by listing protection source ids of labels in this two dimensional array. Using this two dimensional array of label IDs, the cluster generates a list of namespaces to protect, which are derived from intersections of the inner arrays and union of the outer array. This is not supported for object protection on helios.
	...
	// Specifies the user specified timeout in seconds to wait for a volume snapshot to become ready. This is not supported if CSI snapshot is not enabled. Default: 900 secs for IBM baas, 300 secs for others
	SnapshotTimeoutSeconds *int32 `json:"snapshotTimeoutSeconds,omitempty"`
