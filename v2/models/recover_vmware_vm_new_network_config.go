...
type RecoverVmwareVMNewNetworkConfig struct {
	...
	// Specifies the IP address to be assigned to the VM when ipAllocationMode is set to MANUAL. This field is required when ipAllocationMode is MANUAL.
	IPAddress *string `json:"ipAddress,omitempty"`

	// Specifies the IP address allocation mode for the network.
	// Enum: [POOL DHCP MANUAL NONE]
...
var recoverVmwareVmNewNetworkConfigTypeIPAllocationModePropEnum []interface{}

func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["POOL","DHCP","MANUAL","NONE"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
		recoverVmwareVmNewNetworkConfigTypeIPAllocationModePropEnum = append(recoverVmwareVmNewNetworkConfigTypeIPAllocationModePropEnum, v)
	}
}

const (

	// RecoverVmwareVMNewNetworkConfigIPAllocationModePOOL captures enum value "POOL"
	RecoverVmwareVMNewNetworkConfigIPAllocationModePOOL string = "POOL"

	// RecoverVmwareVMNewNetworkConfigIPAllocationModeDHCP captures enum value "DHCP"
	RecoverVmwareVMNewNetworkConfigIPAllocationModeDHCP string = "DHCP"

	// RecoverVmwareVMNewNetworkConfigIPAllocationModeMANUAL captures enum value "MANUAL"
	RecoverVmwareVMNewNetworkConfigIPAllocationModeMANUAL string = "MANUAL"

	// RecoverVmwareVMNewNetworkConfigIPAllocationModeNONE captures enum value "NONE"
	RecoverVmwareVMNewNetworkConfigIPAllocationModeNONE string = "NONE"
)

// prop value enum
func (m *RecoverVmwareVMNewNetworkConfig) validateIPAllocationModeEnum(path, location string, value string) error {
	if err := validate.EnumCase(path, location, value, recoverVmwareVmNewNetworkConfigTypeIPAllocationModePropEnum, true); err != nil {
		return err
	}
	return nil
}

func (m *RecoverVmwareVMNewNetworkConfig) validateIPAllocationMode(formats strfmt.Registry) error {
	if swag.IsZero(m.IPAllocationMode) { // not required
		return nil
	}

	// value enum
	if err := m.validateIPAllocationModeEnum("ipAllocationMode", "body", *m.IPAllocationMode); err != nil {
		return err
	}
