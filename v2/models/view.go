...
type View struct {
	...
	// for WORM views created through legacy flows (Snapshots,
	// Cloning, etc..). WORM status must therefore be determined by
	...
	// Id of the LSU (Logical Storage Unit) this View belongs to.
	// Read Only: true
	LsuID *int64 `json:"lsuId,omitempty"`
...
func (m *View) UnmarshalJSON(raw []byte) error {
	...
	m.DataLockMode = dataAO0.DataLockMode
...
func (m View) MarshalJSON() ([]byte, error) {
	...
	dataAO0.DataLockMode = m.DataLockMode
...
func (m *View) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateDataLockMode(formats); err != nil {
		res = append(res, err)
	}
...
var viewTypeDataLockModePropEnum []interface{}
...
func init() {
	...
	for _, v := range res {
		viewTypeDataLockModePropEnum = append(viewTypeDataLockModePropEnum, v)
	}
}

// property enum
func (m *View) validateDataLockModeEnum(path, location string, value string) error {
	if err := validate.EnumCase(path, location, value, viewTypeDataLockModePropEnum, true); err != nil {
		return err
	}
	return nil
}

func (m *View) validateDataLockMode(formats strfmt.Registry) error {

	if swag.IsZero(m.DataLockMode) { // not required
		return nil
	}

	// value enum
	if err := m.validateDataLockModeEnum("dataLockMode", "body", *m.DataLockMode); err != nil {
		return err
	}
...
func (m *View) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	...
	if err := m.contextValidateLsuID(ctx, formats); err != nil {
		res = append(res, err)
	}
...
func (m *View) contextValidateLsuID(ctx context.Context, formats strfmt.Registry) error {

	if err := validate.ReadOnly(ctx, "lsuId", "body", m.LsuID); err != nil {
		return err
	}
