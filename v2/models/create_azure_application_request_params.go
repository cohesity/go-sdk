...
type CreateAzureApplicationRequestParams struct {
	...
	// Specifies a list of certificate thumbprints. The count of items in this list should be either one or equal to the appCount. If only a single thumbprint is provided, all newly created apps will share the certificate.
	...
	// Specifies the list of exisiting Microsoft365 azure application credentials for which certificates are to be added/ updated. Each app credential in this list should contain a certificate id.
	O365AppCredentialsListForCertUpdate []*Office365AppCredentials `json:"o365AppCredentialsListForCertUpdate"`
...
func (m *CreateAzureApplicationRequestParams) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateO365AppCredentialsListForCertUpdate(formats); err != nil {
		res = append(res, err)
	}
...
func (m *CreateAzureApplicationRequestParams) validateO365AppCredentialsListForCertUpdate(formats strfmt.Registry) error {
	if swag.IsZero(m.O365AppCredentialsListForCertUpdate) { // not required
		return nil
	}

	for i := 0; i < len(m.O365AppCredentialsListForCertUpdate); i++ {
		if swag.IsZero(m.O365AppCredentialsListForCertUpdate[i]) { // not required
			continue
		}

		if m.O365AppCredentialsListForCertUpdate[i] != nil {
			if err := m.O365AppCredentialsListForCertUpdate[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("o365AppCredentialsListForCertUpdate" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("o365AppCredentialsListForCertUpdate" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["kVirtualMachine","kSQL","kEntraID","kFileShare","kKubernetes","kMySQL","kMySQLFlexibleServer","kCosmosDBCassandraAccount","kAzureCosmosDBNoSQLAccount","kAzureCosmosDBMongoDBAccount","kCosmosDBTableAPIAccount","kBlobStorage","kStorageTable","kCosmosDBNoSQLAccount","kAzureSQLMI","kAzureSQLDB","kPostgreSQLFlexibleServer"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
func (m *CreateAzureApplicationRequestParams) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	...
	if err := m.contextValidateO365AppCredentialsListForCertUpdate(ctx, formats); err != nil {
		res = append(res, err)
	}
...
func (m *CreateAzureApplicationRequestParams) contextValidateO365AppCredentialsListForCertUpdate(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.O365AppCredentialsListForCertUpdate); i++ {

		if m.O365AppCredentialsListForCertUpdate[i] != nil {
			if err := m.O365AppCredentialsListForCertUpdate[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("o365AppCredentialsListForCertUpdate" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("o365AppCredentialsListForCertUpdate" + "." + strconv.Itoa(i))
