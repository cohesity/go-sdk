...
type Node struct {
	...
	// 'AutoHealthCheck' means the entity health is bad.
	// 'UserGracefulRemoval' means user initiated a graceful removal.
	// 'UserAvoidAccess' means user initiated a mark offline.
	// 'UserGracefulNodeRemoval' mean users initiated graceful node removal.
	// 'UserRemoveDownNode' mean user initiated graceful removal of down node.
	// 'BridgeDataUnavailable' Bridge requested a graceful removal of a disk when it is not available.
	RemovalReason []string `json:"removalReason"`

	// RemovalState specifies the removal state of the node.
	// 'DontRemove' means the state of object is functional and
	// it is not being removed.
	// 'MarkedForRemoval' means the object is being removed.
	// 'OkToRemove' means the object has been removed on the Cohesity Cluster and
	...
	// TotalDiskBytes specifies the total disk capacity of the
	// node in bytes.
	TotalDiskBytes *int64 `json:"totalDiskBytes,omitempty"`

	// UsedDiskBytes specifies the used disk capacity of the
	// node in bytes.
	UsedDiskBytes *int64 `json:"usedDiskBytes,omitempty"`
