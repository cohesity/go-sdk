...
type ClusterDocsMetadata struct {
	...
	// Enum: [APIDocs BackupServiceConnectionDocs HelpDocs SupportDocs LicenseDocs]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["APIDocs","BackupServiceConnectionDocs","HelpDocs","SupportDocs","LicenseDocs"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
