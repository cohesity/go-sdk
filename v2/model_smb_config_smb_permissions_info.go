/*
Cohesity REST API

Cohesity API provides a RESTful interface to access the various data management operations on Cohesity cluster and Helios.

API version: 2.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package v2

import (
	"encoding/json"
)

// checks if the SmbConfigSmbPermissionsInfo type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SmbConfigSmbPermissionsInfo{}

// SmbConfigSmbPermissionsInfo Specifies the SMB permissions for the View.
type SmbConfigSmbPermissionsInfo struct {
	// Specifies the security identifier (SID) of the owner of the SMB share.
	OwnerSid NullableString `json:"ownerSid,omitempty"`
	// Array of SMB Permissions. Specifies a list of SMB permissions.
	Permissions []SmbPermission `json:"permissions,omitempty"`
}

// NewSmbConfigSmbPermissionsInfo instantiates a new SmbConfigSmbPermissionsInfo object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSmbConfigSmbPermissionsInfo() *SmbConfigSmbPermissionsInfo {
	this := SmbConfigSmbPermissionsInfo{}
	return &this
}

// NewSmbConfigSmbPermissionsInfoWithDefaults instantiates a new SmbConfigSmbPermissionsInfo object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSmbConfigSmbPermissionsInfoWithDefaults() *SmbConfigSmbPermissionsInfo {
	this := SmbConfigSmbPermissionsInfo{}
	return &this
}

// GetOwnerSid returns the OwnerSid field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SmbConfigSmbPermissionsInfo) GetOwnerSid() string {
	if o == nil || IsNil(o.OwnerSid.Get()) {
		var ret string
		return ret
	}
	return *o.OwnerSid.Get()
}

// GetOwnerSidOk returns a tuple with the OwnerSid field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SmbConfigSmbPermissionsInfo) GetOwnerSidOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.OwnerSid.Get(), o.OwnerSid.IsSet()
}

// HasOwnerSid returns a boolean if a field has been set.
func (o *SmbConfigSmbPermissionsInfo) HasOwnerSid() bool {
	if o != nil && o.OwnerSid.IsSet() {
		return true
	}

	return false
}

// SetOwnerSid gets a reference to the given NullableString and assigns it to the OwnerSid field.
func (o *SmbConfigSmbPermissionsInfo) SetOwnerSid(v string) {
	o.OwnerSid.Set(&v)
}
// SetOwnerSidNil sets the value for OwnerSid to be an explicit nil
func (o *SmbConfigSmbPermissionsInfo) SetOwnerSidNil() {
	o.OwnerSid.Set(nil)
}

// UnsetOwnerSid ensures that no value is present for OwnerSid, not even an explicit nil
func (o *SmbConfigSmbPermissionsInfo) UnsetOwnerSid() {
	o.OwnerSid.Unset()
}

// GetPermissions returns the Permissions field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SmbConfigSmbPermissionsInfo) GetPermissions() []SmbPermission {
	if o == nil {
		var ret []SmbPermission
		return ret
	}
	return o.Permissions
}

// GetPermissionsOk returns a tuple with the Permissions field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SmbConfigSmbPermissionsInfo) GetPermissionsOk() ([]SmbPermission, bool) {
	if o == nil || IsNil(o.Permissions) {
		return nil, false
	}
	return o.Permissions, true
}

// HasPermissions returns a boolean if a field has been set.
func (o *SmbConfigSmbPermissionsInfo) HasPermissions() bool {
	if o != nil && !IsNil(o.Permissions) {
		return true
	}

	return false
}

// SetPermissions gets a reference to the given []SmbPermission and assigns it to the Permissions field.
func (o *SmbConfigSmbPermissionsInfo) SetPermissions(v []SmbPermission) {
	o.Permissions = v
}

func (o SmbConfigSmbPermissionsInfo) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SmbConfigSmbPermissionsInfo) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.OwnerSid.IsSet() {
		toSerialize["ownerSid"] = o.OwnerSid.Get()
	}
	if o.Permissions != nil {
		toSerialize["permissions"] = o.Permissions
	}
	return toSerialize, nil
}

type NullableSmbConfigSmbPermissionsInfo struct {
	value *SmbConfigSmbPermissionsInfo
	isSet bool
}

func (v NullableSmbConfigSmbPermissionsInfo) Get() *SmbConfigSmbPermissionsInfo {
	return v.value
}

func (v *NullableSmbConfigSmbPermissionsInfo) Set(val *SmbConfigSmbPermissionsInfo) {
	v.value = val
	v.isSet = true
}

func (v NullableSmbConfigSmbPermissionsInfo) IsSet() bool {
	return v.isSet
}

func (v *NullableSmbConfigSmbPermissionsInfo) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSmbConfigSmbPermissionsInfo(val *SmbConfigSmbPermissionsInfo) *NullableSmbConfigSmbPermissionsInfo {
	return &NullableSmbConfigSmbPermissionsInfo{value: val, isSet: true}
}

func (v NullableSmbConfigSmbPermissionsInfo) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSmbConfigSmbPermissionsInfo) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


