...
type AzureProtectionSource struct {
	...
	DiskInfo []*AzureDiskInfo `json:"diskInfoList"`
	...
	// IsHnsEnabled specifies whether the Storage Account has
	// the Hierarchical Namespace (HNS) feature.
	IsHnsEnabled *bool `json:"isHnsEnabled,omitempty"`

	// IsHnsEnabled specifies whether the Storage Container has
	// the Hierarchical Namespace (HNS) feature.
...
func (m *AzureProtectionSource) validateDiskInfo(formats strfmt.Registry) error {
	if swag.IsZero(m.DiskInfo) { // not required
		return nil
	}

	for i := 0; i < len(m.DiskInfo); i++ {
		if swag.IsZero(m.DiskInfo[i]) { // not required
			continue
		}

		if m.DiskInfo[i] != nil {
			if err := m.DiskInfo[i].Validate(formats); err != nil {
...
func (m *AzureProtectionSource) contextValidateDiskInfo(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.DiskInfo); i++ {

		if m.DiskInfo[i] != nil {
			if err := m.DiskInfo[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("diskInfoList" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("diskInfoList" + "." + strconv.Itoa(i))
