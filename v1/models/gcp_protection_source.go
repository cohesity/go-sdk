...
type GcpProtectionSource struct {
	...
	// Specifies the database size in bytes for Spanner databases.
	...
	Disk []*GcpDiskInfo `json:"gcpDiskInfoList"`
	...
	IPAddressVM *string `json:"ipAddressesVM,omitempty"`
...
func (m *GcpProtectionSource) validateDisk(formats strfmt.Registry) error {
	if swag.IsZero(m.Disk) { // not required
		return nil
	}

	for i := 0; i < len(m.Disk); i++ {
		if swag.IsZero(m.Disk[i]) { // not required
			continue
		}

		if m.Disk[i] != nil {
			if err := m.Disk[i].Validate(formats); err != nil {
...
func (m *GcpProtectionSource) contextValidateDisk(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.Disk); i++ {

		if m.Disk[i] != nil {
			if err := m.Disk[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("gcpDiskInfoList" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("gcpDiskInfoList" + "." + strconv.Itoa(i))
