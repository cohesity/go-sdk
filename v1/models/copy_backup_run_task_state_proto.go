...
type CopyBackupRunTaskStateProto struct {
	...
	// The set of entities for which this copy needs to be made.
	// If there are some entities in this vec, then only those entities will
	// considered for this copytask. If this is empty, then all eligible entities
	// will from the backup run will be considered for this copytask.
	// Note: This is only supported for kRemote snapshot targets for now.
	EntityStringIDVec []string `json:"entityStringIdVec"`
	...
	// OwnershipContext for this copy run. By default all regular copy runs have
	// the value of kOwnershipContextLocal. Copy run targets configured for
	// Onprem FortKnox will have the value of kOwnershipContextOnpremVault.
	// Onprem FortKnox uses the same replication mechanism behind the scene,
	// except a few differences as follows:
	...
	// Reasons for which the copy task is paused. The task will resume only when
	// all the pause reasons are removed.
	PauseReasonVec []int32 `json:"pauseReasonVec"`
	...
	// If this is an source-copy task, this field will contain some basic info
	// specific to the source copy task.
	SourceCopyTaskInfo *SourceSnapshotCopyTaskInfo `json:"sourceCopyTaskInfo,omitempty"`
	...
	// This will store the status of op which was before pausing.
	StatusBeforePause *int32 `json:"statusBeforePause,omitempty"`
	...
	// Will contain any warnings encountered by this task.
...
func (m *CopyBackupRunTaskStateProto) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateSourceCopyTaskInfo(formats); err != nil {
		res = append(res, err)
	}
...
func (m *CopyBackupRunTaskStateProto) validateSourceCopyTaskInfo(formats strfmt.Registry) error {
	if swag.IsZero(m.SourceCopyTaskInfo) { // not required
		return nil
	}

	if m.SourceCopyTaskInfo != nil {
		if err := m.SourceCopyTaskInfo.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("sourceCopyTaskInfo")
			} else if ce, ok := err.(*errors.CompositeError); ok {
				return ce.ValidateName("sourceCopyTaskInfo")
			}
			return err
		}
	}
...
func (m *CopyBackupRunTaskStateProto) validateWarnings(formats strfmt.Registry) error {
...
func (m *CopyBackupRunTaskStateProto) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	...
	if err := m.contextValidateSourceCopyTaskInfo(ctx, formats); err != nil {
		res = append(res, err)
	}
...
func (m *CopyBackupRunTaskStateProto) contextValidateSourceCopyTaskInfo(ctx context.Context, formats strfmt.Registry) error {
	...
	if m.SourceCopyTaskInfo != nil {
		...
		if err := m.SourceCopyTaskInfo.ContextValidate(ctx, formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("sourceCopyTaskInfo")
			} else if ce, ok := err.(*errors.CompositeError); ok {
				return ce.ValidateName("sourceCopyTaskInfo")
...
func (m *CopyBackupRunTaskStateProto) contextValidateWarnings(ctx context.Context, formats strfmt.Registry) error {
