...
type DeviceTreeDetails struct {
	...
	// Specifies internal device identifier of the device to be
	...
	// Specifies the chunk size (in bytes) used by the thin pool for the
	// LVM volume (for which this device tree is being built). This defines
	// the granularity at which space is allocated for thin-provisioned volumes
	// and does not include LVM metadata overhead.
