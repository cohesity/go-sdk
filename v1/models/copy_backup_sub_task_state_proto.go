...
type CopyBackupSubTaskStateProto struct {
	...
	// The time this task is scheduled.
	...
	// Stores the task status prior to a pause operation, enabling correct
	// resumption logic. When resuming a paused subtask, execution always
	// restarts from the kStarted state. This field records the original
	// status to guide special handling required for certain states
	// (e.g., invoking a resume API when the prior state was kAccepted,
	// instead of the normal start logic).
