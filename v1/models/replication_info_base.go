...
type ReplicationInfoBase struct {
	...
	// Indicates if this task can be scheduled in parallel to the other tasks in
	// this backup job. This will be set to true if the corresponding backup job
	// supports replicating snapshots in parallel and Rx is also upgraded to the
	// version which supports parallel replication.
	IsParallelReplicationSupported *bool `json:"isParallelReplicationSupported,omitempty"`
	...
	// Progress monitor task path constructed by Bridge
	// Format: replication_<job_id>_<instance_id>/task_<replication_uid_object_id>
	ProgressMonitorTaskPath *string `json:"progressMonitorTaskPath,omitempty"`

	// Pre-formatted detailed progress string from Bridge
	// for hierarchical Pulse progress reporting.
	PulseDetailedProgress *string `json:"pulseDetailedProgress,omitempty"`
