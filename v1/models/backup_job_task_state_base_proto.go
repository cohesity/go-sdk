...
type BackupJobTaskStateBaseProto struct {
	...
	// The base scheduled backup type before any conversion (e.g., kRegular
	// to kFull for UDAv2 adapters that don't support incremental backups).
	// If not set, it means no conversion happened and backup_type is the
	// original. This is used for scheduling tracking to ensure the scheduler
	// uses the original scheduled type when determining the next backup time.
	BaseScheduledBackupType *int32 `json:"baseScheduledBackupType,omitempty"`
	...
	// Total number of granular objects protected in this backup.
	NumProtectedGranularObjects *int64 `json:"numProtectedGranularObjects,omitempty"`

	// Number of changed granular objects which were backed up succesfully.
	...
	// The start time for the run this task belongs to.
	...
	// Indicates whether View Module is to be used for the view management of
	// this run/attempt/task.
	UseViewModule *bool `json:"useViewModule,omitempty"`
