...
type ViewBox struct {
	...
	// Indicates whether the storage domain supports workload/throughput
	// optimized performance settings for its views.
	// Note: This value cannot be modified after the storage domain is created.
	// Read Only: true
	OptimizeThroughputSettings *bool `json:"optimizeThroughputSettings,omitempty"`
	...
	// Specifies the purpose of the Storage Domain. It acts as a collection of
	// optimal settings for certain workloads. This can only be set during
	// creation.
...
func (m *ViewBox) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	...
	if err := m.contextValidateOptimizeThroughputSettings(ctx, formats); err != nil {
		res = append(res, err)
	}
...
func (m *ViewBox) contextValidateOptimizeThroughputSettings(ctx context.Context, formats strfmt.Registry) error {

	if err := validate.ReadOnly(ctx, "optimizeThroughputSettings", "body", m.OptimizeThroughputSettings); err != nil {
		return err
	}
