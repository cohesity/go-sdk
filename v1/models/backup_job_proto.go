...
type BackupJobProto struct {
	...
	// Backup job params defined for generic adapter.
	// 7.2.2 used tag 101 for this.
	// We renumbered this to 107 at a later time.
	//
	// This field must be used only through provided utility functions to handle
	// backwards incompatibility issues due to the re-numbering.
	AdditionalBackupJobParamsDontUseWithoutUtilityFunc []uint8 `json:"additionalBackupJobParamsDontUseWithoutUtilityFunc"`
	...
	// Indicates whether the backup should be failed on encountering an error.
	// Currently used for physical file based backups.
	...
	// If user need to store extended attributes in backup,
	// this should be enabled. This apply to all NAS related volumes.
	...
	// Paused note provided by user containing the reasoning for pausing future
	// runs if any.
	...
	// Go protobuf doesn't support unknown fields. So, we declare a bytes field
	// in-lieu of marking reserved.
	ReservedField101 []uint8 `json:"reservedField_101"`
	...
	// This field must be used only through the utility functions in
	// magneto/base/utils.h due to protobuf incompatibility issues between
	// 7.1.2/7.2.2 timeframe (ENG-541211).
	//
	// In 7.1.2, this field did not exist.
	// In 7.2.2, this field was using tag 100.
	//
	// We renumbered this to 105 at a later time. But we need to provide
	// backwards compatibility with existing 7.2.2 clusters that have this field
	// set. The getter utility function checks for tag 100 if tag 105 is not
	// present.
	UseViewModuleDontUseWithoutUtilityFunc *bool `json:"useViewModuleDontUseWithoutUtilityFunc,omitempty"`
