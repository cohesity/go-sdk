...
type BackupJobProto struct {
	...
	AdditionalBackupJobParams []uint8 `json:"additionalBackupJobParams"`
	...
	// Indicates whether view module should be used to manage views for this
	// backup job. This is required because currently we do not support older
	// jobs switching over to using View Module for view management. So for any
	// adapter that gets onboarded to View Module, there would be newer jobs that
	// will leverage view module and older ones will not. This field would be set
	// to true by magneto internally for the new jobs only without the user's
	// intervention.
