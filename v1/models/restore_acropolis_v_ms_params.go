...
type RestoreAcropolisVMsParams struct {
	...
	// Whether to restore attached Volume Groups for this VM. Applies to both PE
	// and PC environments.
	RestoreDirectlyAttachedVgs *bool `json:"restoreDirectlyAttachedVgs,omitempty"`
	...
	// The Prism Element cluster where the VM should be restored to. This
	// field is optional if the VM is being restored to its original parent
	// source. If not specified, the VM will be restored to their
	// original PE cluster. This field is mandatory if the VMs are
	// being restored to a different parent source.
	// It is only populated with PE under a PC source not a standalone PE.
	TargetPeCluster *EntityProto `json:"targetPeCluster,omitempty"`
...
func (m *RestoreAcropolisVMsParams) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateTargetPeCluster(formats); err != nil {
		res = append(res, err)
	}
...
func (m *RestoreAcropolisVMsParams) validateTargetPeCluster(formats strfmt.Registry) error {
	if swag.IsZero(m.TargetPeCluster) { // not required
		return nil
	}

	if m.TargetPeCluster != nil {
		if err := m.TargetPeCluster.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("targetPeCluster")
			} else if ce, ok := err.(*errors.CompositeError); ok {
				return ce.ValidateName("targetPeCluster")
			}
			return err
		}
	}
...
func (m *RestoreAcropolisVMsParams) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	...
	if err := m.contextValidateTargetPeCluster(ctx, formats); err != nil {
		res = append(res, err)
	}
...
func (m *RestoreAcropolisVMsParams) contextValidateTargetPeCluster(ctx context.Context, formats strfmt.Registry) error {
	...
	if m.TargetPeCluster != nil {
		...
		if err := m.TargetPeCluster.ContextValidate(ctx, formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("targetPeCluster")
			} else if ce, ok := err.(*errors.CompositeError); ok {
				return ce.ValidateName("targetPeCluster")
