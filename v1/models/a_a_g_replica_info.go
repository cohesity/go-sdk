...
type AAGReplicaInfo struct {
	...
	// Enum: [kSync kAsync]
	AvailabiltyMode *string `json:"availabilityMode,omitempty"`

	// Specifies the backup priority.
	BackupPriority *int32 `json:"lastModifiedMsecs,omitempty"`

	// Specifies the time when replica is created.
	CreatedDateMsecs *int64 `json:"createDateMsecs,omitempty"`
...
func (m *AAGReplicaInfo) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateAvailabiltyMode(formats); err != nil {
		res = append(res, err)
	}
...
var aAGReplicaInfoTypeAvailabiltyModePropEnum []interface{}
...
func init() {
	...
	for _, v := range res {
		aAGReplicaInfoTypeAvailabiltyModePropEnum = append(aAGReplicaInfoTypeAvailabiltyModePropEnum, v)
	}
}

const (

	// AAGReplicaInfoAvailabiltyModeKSync captures enum value "kSync"
	AAGReplicaInfoAvailabiltyModeKSync string = "kSync"

	// AAGReplicaInfoAvailabiltyModeKAsync captures enum value "kAsync"
	AAGReplicaInfoAvailabiltyModeKAsync string = "kAsync"
)

// prop value enum
func (m *AAGReplicaInfo) validateAvailabiltyModeEnum(path, location string, value string) error {
	if err := validate.EnumCase(path, location, value, aAGReplicaInfoTypeAvailabiltyModePropEnum, true); err != nil {
		return err
	}
	return nil
}

func (m *AAGReplicaInfo) validateAvailabiltyMode(formats strfmt.Registry) error {
	if swag.IsZero(m.AvailabiltyMode) { // not required
		return nil
	}

	// value enum
	if err := m.validateAvailabiltyModeEnum("availabilityMode", "body", *m.AvailabiltyMode); err != nil {
		return err
	}
