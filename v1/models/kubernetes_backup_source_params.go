...
type KubernetesBackupSourceParams struct {
	...
	// Boolean to represent if backup should fail if any one of the hook
	// execution has failed.
	FailBackupsOnHookExecErrors *bool `json:"failBackupsOnHookExecErrors,omitempty"`

	// If enabled we will only backup pvc and related resources.
	PvcOnlyBackup *bool `json:"pvcOnlyBackup,omitempty"`

	// List of groups to execute during PVC snapshots.
	QuiesceGroups []*QuiesceGroup `json:"quiesceGroups"`

	// Info about PVC(s) and resource types to be excluded from the from the
	// backup job for the source. When set, this will override
	// KubernetesEnvParams label based PVC filter.
	ExcludeParams *K8SFilterParams `json:"excludeParams,omitempty"`

	// Info about PVC(s) and resource types  to be included from the from the
	// backup job for the source. When set, this will override
	// KubernetesEnvParams label based PVC filter.
