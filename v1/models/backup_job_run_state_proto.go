...
type BackupJobRunStateProto struct {
	...
	// Total number of application objects whose backup was skipped.
	NumSkippedAppObjects *int32 `json:"numSkippedAppObjects,omitempty"`
	...
	// This will be populated if the current run is created from a
	// RunOnceBackupJobProto. Currently, the override_param_vec in this proto is
	// used to override the params for sources in this backup run.
	// TODO(ritvik): Consider deprecating the above additional_param_vec in favor
	// of using this run_once_proto.
	RunOnceProto *RunOnceBackupJobProto `json:"runOnceProto,omitempty"`
...
func (m *BackupJobRunStateProto) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateRunOnceProto(formats); err != nil {
		res = append(res, err)
	}
...
func (m *BackupJobRunStateProto) validateRunOnceProto(formats strfmt.Registry) error {
	if swag.IsZero(m.RunOnceProto) { // not required
		return nil
	}

	if m.RunOnceProto != nil {
		if err := m.RunOnceProto.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("runOnceProto")
			} else if ce, ok := err.(*errors.CompositeError); ok {
				return ce.ValidateName("runOnceProto")
			}
			return err
		}
	}
...
func (m *BackupJobRunStateProto) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	...
	if err := m.contextValidateRunOnceProto(ctx, formats); err != nil {
		res = append(res, err)
	}
...
func (m *BackupJobRunStateProto) contextValidateRunOnceProto(ctx context.Context, formats strfmt.Registry) error {
	...
	if m.RunOnceProto != nil {
		...
		if err := m.RunOnceProto.ContextValidate(ctx, formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("runOnceProto")
			} else if ce, ok := err.(*errors.CompositeError); ok {
				return ce.ValidateName("runOnceProto")
